...
router.post("/home", async(req, res)=>{
    try {
        ...
        return res.status(200).json({ ResponseCode: 200, Result:true, message: "Data Load successful", general:{site_currency:general[0].site_currency}, cus_rating:cus_rating[0], 
    ...
    try {
        ...
        if (mrole == "1") {
            ...
            if (parseFloat(vehicle[0].minimum_fare) >= parseFloat(drop_price)) {
                ...
                return res.status(200).json({ ResponseCode: 200, Result:true, message: `The fare in this city exceeds our maximum limit of ${general[0].site_currency}${vehicle[0].minimum_fare}.`, 
                ...
                return res.status(200).json({ ResponseCode: 200, Result:true, message: `The fare in this city exceeds our minimum limit of ${general[0].site_currency}${vehicle[0].maximum_fare}.`, 
                    offer_expire_time: general[0].offer_expire_time, zoneresult, tot_km:0, drop_price:0, tot_hour: 0, tot_minute: 0, tot_second: 0, driver_id: [], vehicle:vehicle[0]});
            } else {
...
router.post("/module_calculate", async(req, res)=>{
    try {
        ...
        for (let c = 0; c < vehicle.length;) {
            ...
            caldriver.push({...vehicle[c], dri_pic_time: veh, dri_pic_drop: addtime, drop_price:parseFloat(parseFloat(drop_price).toFixed(2)), drop_hour:hou_min.hour, drop_time:hou_min.minute, 
